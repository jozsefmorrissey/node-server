...
//---------------------  Compile Functions ---------------//
    ...
    function toJsFormat(string) {
      let obj, matches;
      while (matches = tagReg.recursive(string)) {
        const shortest = matches.min(str => str.length);
        obj = tagReg.object(shortest, 'tagName', 'attrs', 'contents');
        let {contents, tagName, attrs} = obj
        const repeatObj = repeatReg.object(attrs, null, 'variables', 'repeatScope');
    ...
    if (!$t.functions[id]) {
      if (!$t.templates[id]) {
        ...
        template = toJsFormat(template);
        $t.templates[id] = compile();
      }
    }

    const endTagReg = /( \+) /g;
    this.compiled = function () { return $t.templates[id].replace(endTagReg, '$1\n\t\t');}
    this.render = render;
    this.type = type;
    this.isolateBlocks = isolateBlocks;
    this.id = () => id;
		this.refresh = (scope, selector, nearByElem) => {
			if (!du) du = require('./dom-utils');
			if (!selector) selector = `[t-id='${this.id()}']`;
			const elem = du.find.closest(selector, nearByElem);
			if (elem !== null) {
				this.trigger.before.render()
				elem.innerHTML = this.render(scope);
				instance.trigger.after.render(scope);
			}
		}
  }
}
