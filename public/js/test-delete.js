  ...
  constructor(name, options, notify, stages, alwaysPossible) {
    ...
    this.onClose = function (start, end) {
      return function (str, start, end) {
        if (start >= end && notify) notify(this);
...
function parse(exprDef, str) {
  ...
  progress = [-3, -2, -1];
  while (topOfStack() !== undefined && index < str.length) {
...
//---------------------  Compile Functions ---------------//
        ...
        let scope = tagContents.indexOf('t-scope=') === -1 ? 'scope' :
              tagContents.replace(/.*\$t-scope=('|")([a-zA-Z-_\/]*?)(\1).*/, '$2');
...
const testData = [{
  ...
  built: '`<div>` + (get("key")) + ` => ` + (get("value")) + `</div>`',
  compiled: '<div>key1 => value1</div><div>key2 => value2</div><div>key3 => value3</div><div>key4 => value4</div>',
  numberOfBlocks: 1,
...
const startIndex = 0;
const endIndex = testData.length;
