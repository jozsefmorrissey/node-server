...
exports.downloadLink = (user, order, csg, options) => {
  const dxfStr = serialize(options, csg);
  const dateStr = `${new Date().toLocaleDateString().replace(/\//g, '-')}`;
  const fileId = `${user.id()} ${order.name} ${dateStr}`;
  const relUrl = `/Downloads/dxf/${fileId}.dxf`;
  fs.writeFileSync(`./services/cabinet/public${relUrl}`, dxfStr);
  return relUrl;
}

function incrementalAdd(intial, increment, percentGrowth) {
  let total = 0;
  let cost = increment;
  ...
  return {
    ...
    adjustBalance: ((value) => total += value),
    total: () => total,
    cost: () => cost
  }
}
