...
CustomEvent.all(OpenSectionDisplay, 'change');
du.on.match('input', '.obj-layout-cnt', OpenSectionDisplay.trigger.change);
...
const onDividerTypeChange = (target) => {
  const opening = OpenSectionDisplay.getOpening(target);
  opening.divider().type(target.value)
}

const onSectionTypeChange = (target) => {
  const opening = OpenSectionDisplay.getOpening(target);
  opening.setSection(target.value.toCamelCap() + 'Section');
  opening.getRoot().hash();
}
...
du.on.match('keyup', '.division-pattern-input', onPatternChange);
du.on.match('change', '.divider-type-selector', onDividerTypeChange);
du.on.match('change', '.section-type-selector', onSectionTypeChange);
