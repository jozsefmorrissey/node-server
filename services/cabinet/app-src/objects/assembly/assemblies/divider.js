...
class Divider extends Assembly {
  constructor(partCode, partName, config) {
    ...
    this.frameThickness = this.value.getterSetter('dft');
    ...
    this.panelThickness = this.value.getterSetter('dpt');

    const validScribe = (scribe) => {
    ...
    this.scribe = this.value.getterSetter('sc', false, validScribe);
    this.scribe.valid = validScribe;
